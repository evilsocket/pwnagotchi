...
class BTNap:
    ...
    def is_napping(self, dev_remote):
        """
        Check if already napping
        """
        logging.debug("BT-TETHER: Checking if device is napping")

        try:
            dev_remote.ConnectProfile('nap')
            return True
        except dbus.exceptions.DBusException as err:
            return err.get_dbus_name() == 'org.bluez.Error.AlreadyConnected'
        except Exception:
            pass
        return False
    ...
    @staticmethod
    def pair(device):
        logging.info('BT-TETHER: Trying to pair ...')
        try:
            device.Pair()
            ...
            if err.get_dbus_name() == 'org.bluez.Error.AlreadyExists':
                logging.info('BT-TETHER: Already paired ...')
                return True
        except Exception:
            pass
        return False
    ...
    @staticmethod
    def nap(device):
        logging.info('BT-TETHER: Trying to nap ...')

        try:
            logging.info('BT-TETHER: Connecting to profile ...')
        ...
        try:
            logging.info('BT-TETHER: Connecting to nap network ...')
            net.Connect('nap')
            return True
        except dbus.exceptions.DBusException as err:
    ...
    if READY:
        ...
        if connected:
            ...
            if paired:
                ...
                if bt.is_napping(dev_remote):
                    logging.debug('BT-TETHER: Already napping.')
                    ui.set('bluetooth', 'C')
                else:
                    # connected and paired but not napping
                    logging.info('BT-TETHER: Try to nap ...')
